...
const MintDialog = ({ isOpen, onCloseFunc, refresh }) => {
    ...
    const onMint = async () => {
        try {
            ...
            if (tokenContract && artURI) {
                await tokenContract.methods.mintNFT(artURI).send({ from: accounts[0] });
                refresh();
            }
    ...
    return (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 backdrop-filter backdrop-blur-sm">
            <div className="bg-gray-900 rounded-lg shadow-lg p-8 w-96 relative">
                ...
                <h2 className="text-gray-200 text-2xl font-bold mb-6">Mint Art</h2>
                <div className="mb-6">
                    <InputLabel
                        className='py-2'
                        htmlFor="art_uri"
                    ...
                    <TextInput
                        ...
                        type="text"
                        value={artURI}
                        className="block w-full pl-3 pr-2 py-2 bg-gray-900 text-white border border-gray-700 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
