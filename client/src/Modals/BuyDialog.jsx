...
const BuyDialog = ({ isOpen, onCloseFunc, refresh, art }) => {
    ...
    const onBuy = async () => {
        try {
            ...
            const contract = await getTokenContract(web3Instance);
            const accounts = await web3Instance.eth.getAccounts();

            const weiAmount = Web3.utils.toWei(art.price, 'ether');
            const balanceWei = await web3Instance.eth.getBalance(accounts[0]);

            if (weiAmount > balanceWei) {
                setExceedMsg("Your current Ether balance is not enough")
                return;
            }
            ...
            await contract.methods.buyNFT(art.id).send({from: accounts[0], value: weiAmount, gas:gasLimit});
            refresh();
    ...
    return (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 backdrop-filter backdrop-blur-sm">
            <div className="bg-gray-900 rounded-lg shadow-lg p-8 w-96 relative">
                ...
                <h2 className="text-gray-200 text-2xl font-bold mb-6">Buy NFT</h2>
                <div className="mb-6">
                    <InputLabel
                        className='py-2'
                        htmlFor="get_balance"
